    ...
    for j in range(len(data)):
        for i in range(len(data[j])):
            if data[j][i] == 'True':
                ...
                data[j][i] = False
            elif is_number(data[j][i]):
                data[j][i] = float(data[j][i])
                if data[j][i] % 1 == 0.0:
                    data[j][i] = int(data[j][i])                
            elif data[j][i] == 'null' or data[j][i] == '':
                data[j][i] = None
...
def get_station_info(station_id: int, stations: List[list]) -> list:
    ...
    for station in stations:
        if station[ID] == station_id:
            info = [station[NAME], station[BIKES_AVAILABLE], station[DOCKS_AVAILABLE]] 
    return info
...
def get_station_with_max_bikes(stations: List[list]) -> int:
    ...
    maxva = 0
    for station in stations:
        if station[BIKES_AVAILABLE] > maxva:
            maxva = station[BIKES_AVAILABLE]
    for statio in stations:
        if statio[BIKES_AVAILABLE] == maxva:
            return statio[ID]
...
def get_direction(start_id: int, end_id: int, stations: List[list]) -> str:
    ...
    if lo1 > lo2:
        ...
        h_direction = ''
    return v_direction + h_direction 
...
def balance_all_bikes(stations: List[list]) -> int:
    ...
    bikerent = 0
    bikereturn = 0
    totalbike = get_total(BIKES_AVAILABLE, stations)
    totalcapacity = get_total(CAPACITY, stations)
    avg = totalbike / totalcapacity
    for station in stations:
        if station[BIKES_AVAILABLE] > round(station[CAPACITY] * avg):
            while station[BIKES_AVAILABLE] != round(station[CAPACITY] * avg) and station[IS_RENTING]:
                rent_bike(station[ID], stations)
                bikerent += 1  
        if station[BIKES_AVAILABLE] < round(station[CAPACITY] * avg):
            while station[BIKES_AVAILABLE] != round(station[CAPACITY] * avg) and station[IS_RETURNING]:        
                return_bike(station[ID], stations)
                bikereturn += 1
    difference = bikerent - bikereturn
    return difference
